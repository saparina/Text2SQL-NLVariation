...
def _train(opt):
    ...
    tokenizer_class = XLMRobertaTokenizerFast if "xlm" in opt.model_type else RobertaTokenizerFast
    ...
    if opt.model_name_or_path.find('checkpoint') >= 0:
        ...
        model.load_state_dict(torch.load(opt.save_path + "/dense_classifier.pt"))
    else:
        # initialize model
        model = MyClassifier(
    ...
    resume_train_step, resume_epoch = None, None
    if opt.model_name_or_path.find('checkpoint') >= 0:
    ...
    for epoch in range(opt.epochs):
        ...
        for batch in train_dataloder:
            ...
            if train_step % num_checkpoint_steps == 0:
                ...
                os.makedirs(os.path.join(opt.save_path, 'last_checkpoint'), exist_ok = True)
                torch.save(model.state_dict(), os.path.join(opt.save_path, 'last_checkpoint') + "/dense_classifier.pt")
                tokenizer.save_pretrained(save_directory = os.path.join(opt.save_path, 'last_checkpoint'))
                ...
                torch.save(optimizer_scheduler, os.path.join(opt.save_path, 'last_checkpoint', "optimizer_scheduler.pth"))
                ...
                torch.save(rng_states, os.path.join(opt.save_path, 'last_checkpoint', "rng_states.pth"))
