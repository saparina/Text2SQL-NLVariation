...
class MyClassifier(nn.Module):
    def __init__(
        ...
        model_class = XLMRobertaModel if "xlm" in model_type else RobertaModel
        if mode in ["eval", "test"]:
            # load config
            config = AutoConfig.from_pretrained(model_name_or_path, cache_dir="/transformers_cache/")
    ...
    def table_column_cross_attention(
        ...
        for table_id in range(table_num):
            ...
            assert self.table_column_cross_attention_layer.batch_first is False
            table_name_embedding_attn, _ = self.table_column_cross_attention_layer(
                table_name_embedding.unsqueeze(1),
                column_info_embeddings_in_one_table.unsqueeze(1),
                column_info_embeddings_in_one_table.unsqueeze(1)
            )
            table_name_embedding_attn = table_name_embedding_attn.squeeze(1)
    ...
    def table_column_cls(
        ...
        for batch_id in range(batch_size):
            ...
            for table_name_ids in aligned_table_name_ids:
                table_name_embeddings = sequence_embeddings[table_name_ids, :].unsqueeze(1)
                assert self.table_name_bilstm.batch_first is False
                
                # BiLSTM pooling
                output_t, (hidden_state_t, _) = self.table_name_bilstm(table_name_embeddings)
                output_t = output_t.squeeze(1)
                hidden_state_t = hidden_state_t.squeeze(1)
            ...
            for column_info_ids in aligned_column_info_ids:
                column_info_embeddings = sequence_embeddings[column_info_ids, :].unsqueeze(1)
                assert self.column_info_bilstm.batch_first is False
                
                # BiLSTM pooling
                _, (hidden_state_c, _) = self.column_info_bilstm(column_info_embeddings)
                hidden_state_c = hidden_state_c.squeeze(1)
